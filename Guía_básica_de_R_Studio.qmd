...
format: 
  revealjs:
    footer: "Guía básica R Studio"
    incremental: true
    scrollable: true
    theme: sky
    logo: huca.png
    slide-number: true  # Activa la numeración de diapositivas
...
- **Categóricas**: Representan categorías o grupos y pueden ser:
  ...
  - **Ordinales**: Tienen un orden jerárquico (ejemplo: estadios de una enfermedad: leve, moderado, grave).
    ...
    ```r
    data$gravedad <- factor(data$gravedad, labels = c("leve", "moderado", "grave"), ordered = TRUE)  
